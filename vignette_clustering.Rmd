...
```{r, eval = FALSE}
g <- g %>% spectral_cluster(sigma = 100, k = k, nreps = nreps)
```
...
The similar clusterings over time reflect the fact that tracts don't change demographics very quickly over the time period we are considering, and so clusters tend to be quite persistent. 

However, there's also at least one interesting signal in the clusters. For example, the clustering detects that the historically white-dominated area centered at Grosse Point (Cluster 1) has  receded from 1990 to 2010, with its western-most areas being "absorbed" into a predominantly black cluster. 
...
Unlike spectral clustering, hierarchical clustering picked up on a pretty interesting trend: the expanding yellow cluster in the southwest reflects gradual black settlement in the historically-white suburbs in the form of Cluster 4.  
