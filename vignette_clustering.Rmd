...
library(scales)
library(RSpectra)     # fast eigenvalues
```
...
```{r, fig.height = 4}
A   <- affinity_matrix(g, sigma = 50)        # square exponential affinity with specified sigma
...
```
 ...
 The algorithm's behavior reflects the fact that tracts don't change demographics very quickly over the time period we are considering, and so clusters tend to be quite persistent. 

However, there's also a substantial amount of signal in the clusters. For example, the clustering detects that the historically white community of Grosse Point (the far eastern tip) has somewhat receded from 1990 to 2010, with its western-most areas being "absorbed" into a predominantly black cluster. 
...
We can get a sense of where to split the result by looking at `height` element of `a`: 
```{r}
plot(rev(max(a$height) - a$height), log = 'x')
```

```{r}
data_frame(height = a$height, 
		   n = length(a$height):1) %>% 
...
nodes <- data_frame(geoid = V(g)$geoid,
						x = V(g)$x, 
					y = V(g)$y, 
					t = V(g)$t, 
					cluster = V(g)$cluster)
...
Priorities for future work on `compx` include refinement clustering methods, performance enhancements, and improvements TBD based on suggestions from those in the planning and sociological communities. A particular future improvement would be the addition of a factor that increased the importance of temporal differences, allowing spectral methods to find more dynamic structure. 
